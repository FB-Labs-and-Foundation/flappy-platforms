...
namespace AudioManager.Runtime.Core.Manager
{
	public class ManagerAudio : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			{
				if (this.dynamicSettingsListenerList[i].audioSource == null ||
				    !this.dynamicSettingsListenerList[i].audioSource.isPlaying)
				{
					this.dynamicSettingsListenerList.RemoveAt(i);
				}
				else
				{
					if (this.audioSourcesVolumeChangeList.All(a =>
						    a.audioSource != this.dynamicSettingsListenerList[i].audioSource))
					{
		...
		{
			var musicVolume = PlayerPrefs.GetFloat(managerAudioConfig.MusicVolumeKey, 1);
			UpdateMusicVolume(musicVolume);

			var soundsVolume = PlayerPrefs.GetFloat(managerAudioConfig.SoundVolumeKey, 1);
			UpdateSoundVolume(soundsVolume);
		}
		...
		{
			var volume = -80 + (80 * value);
			var mixer = managerAudioConfig.GetAudioMixer();
			mixer.SetFloat(managerAudioConfig.MusicVolumeKey, volume);
		}

		public void SwitchMusicState(bool value)
		{
			var volume = value ? 1f : 0f;
		...
		{
			PlayerPrefs.SetFloat(managerAudioConfig.MusicVolumeKey, volume);
			PlayerPrefs.Save();
		}
		...
		public void UpdateSoundVolume(float value)
		{
			var volume = -80 + (80 * value);
			var mixer = managerAudioConfig.GetAudioMixer();
			mixer.SetFloat(managerAudioConfig.SoundVolumeKey, volume);
		}

		public void SwitchSoundsState(bool value)
		{
			var volume = value ? 1f : 0f;
			UpdateSoundVolume(volume);
			ApplySoundsVolume(volume);
		}

		public void ApplySoundsVolume(float volume)
		{
			PlayerPrefs.SetFloat(managerAudioConfig.SoundVolumeKey, volume);
			PlayerPrefs.Save();
		}
